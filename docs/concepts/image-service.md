...
- **Hardening.** Every response carries `nosniff`. The restrictive `sandbox` CSP is applied to types that could smuggle script into the delivery origin (SVG, XML, HTML) so user-uploaded SVGs stay safe to serve inline, while media and PDFs are served without it — `sandbox` breaks the browser's built-in PDF viewer.
