...
`b10cks:upgrade` also reconciles the seeded system roles with the shipped role definitions (`authorization:sync-roles`; run it directly with `--dry-run` to preview or `--prune` to also remove roles dropped from the config). Custom roles are never touched — add newly introduced abilities to them by hand where wanted.
:::
