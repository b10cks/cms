...
if [ -z "$VERSION" ]; then
    # Pin to a concrete tag rather than riding :latest, so a reinstall six months
    # from now reproduces the same stack. Falls back to main + :latest offline.
...
if [ -z "$VERSION" ]; then
    say "could not reach the GitHub API — falling back to the main branch"
    REF="main"
    IMAGE_TAG="latest"
else
    REF="$VERSION"
...
DB_PASSWORD="$(rand)"
[ -n "$DB_PASSWORD" ] || die "could not generate a database password."
