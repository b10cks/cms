...
class TranslationStreamController extends Controller
{
    ...
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        return new StreamedResponse(function () use ($space, $systemPrompt, $userPrompt) {
            if (ob_get_level() == 0) {
                ob_start();
            }
            ...
            try {
                foreach ($this->streamService->streamWithSystemPrompt($space, $systemPrompt, $userPrompt) as $event) {
                    $now = time();
