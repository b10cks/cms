...
class ContentTreeOperationService
{
    ...
    public function createItem(
        ...
        return DB::transaction(function () use ($parent, $attributes, $space, $owner): array {
            ...
            $enabledLanguages = collect($space->settings->getEnabledLanguages())
                ...
                ->values();
            $parentFamily = $parent
                ? $this->contentI18nService->getFamily($parent)->keyBy('language_iso')
                : collect();
            $canonicalParent = $parent
                ? ($parentFamily->get($defaultLanguage) ?? $parent)
                : null;
            $warnings = [];
            ...
            ], $canonical, $space, $owner);

            $created = collect([$canonical->fresh()]);

            foreach ($enabledLanguages as $languageIso) {
                if ($languageIso === $defaultLanguage) {
                    continue;
                }

                $translatedParent = $parent ? $parentFamily->get($languageIso) : null;

                if ($parent && ! $translatedParent) {
                    $warnings[] = $this->makeWarning(
                        'missing-parent-translation',
                        $languageIso,
                        null,
                        'Skipped a translated create because the destination parent does not exist in that language.'
                    );
                    continue;
                }
                ...
                $this->createContent->execute([
                    ...
                    'content' => [],
                    'settings' => $attributes['settings'] ?? [],
                ], $translated, $space, $owner);

                $created->push($translated->fresh());
            }
    ...
    public function moveItems(
        ...
        return DB::transaction(function () use ($orderedIds, $parentId, $afterId, $space): array {
            ...
            $targetParent = $parentId
                ...
                : null;
            $after = $afterId
                ? Content::query()->whereNull('deleted_at')->findOrFail($afterId)
                : null;
            ...
            $parentFamily = $targetParent
                ...
                : collect();
            $afterFamily = $after
                ? $this->contentI18nService->getFamily($after)->keyBy('language_iso')
                : collect();
            $languages = collect($familyCache)
                ->flatMap(fn (Collection $family) => $family->pluck('language_iso'))
                ->unique()
            ...
            foreach ($languages as $languageIso) {
                ...
                $this->ensureAfterTargetIsCompatible(
                    $translatedItems,
                    $translatedParent?->id,
                    $translatedAfter,
                );
    ...
    public function deleteSubtrees(
        array $ids,
        Space $space,
    ): array {
    ...
    public function resolveOrderedRootSelection(array $ids): array
    {
        ...
        $selectedIds = collect($ids)
            ->filter(fn (string $id) => $items->has($id))
            ->values();
            ...
            ->filter(function (string $id) use ($selectedIds, $items): bool {
                ...
                while ($current?->parent_id) {
                    ...
                    $current = $items->get($current->parent_id)
                        ?? Content::query()->whereNull('deleted_at')->find($current->parent_id);
                }
    ...
    protected function ensureAfterTargetIsCompatible(
        ...
        $allMovingIds = collect($movingIds)
            ->concat($items->pluck('id'))
            ->filter()
            ->unique()
            ->all();
